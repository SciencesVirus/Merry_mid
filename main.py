...
def run_level(screen, camera_manager, fullscreen_manager, level_name, selected_sound_pack):
    ...
    while running:
        ...
        #     mp_drawing.draw_landmarks(cropped_frame, result.pose_landmarks, mp_pose.POSE_CONNECTIONS, landmark_drawing_spec=mp_drawing_styles.get_default_pose_landmarks_style())
            ...
            if can_play_sound and not sound_triggered_in_current_window:
                ...
                if resource and resource["sound"]:
                    ...
                    if action_detected:
                        ...
                        # --- 狀態更新移到這裡 ---
                        # 只有在音效成功播放後，才更新對應的 prev_ 狀態
                        if current_action_name == "open": prev_open = True
                        elif current_action_name.startswith("head"): prev_head = True
                        elif current_action_name == "left": prev_left = True
                        elif current_action_name == "right": prev_right = True
        ...
        # 繪製攝像頭切換按鈕
        canvas, camera_switch_button_rect = camera_manager.draw_switch_button(canvas, cam_x, cam_y, cam_w, cam_h)
        ...
        game_surface = pygame.image.frombuffer(cv2.cvtColor(canvas, cv2.COLOR_BGR2RGB).tobytes(), (SCREEN_WIDTH, SCREEN_HEIGHT), "RGB")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if show_finish_popup:
                    if next2_btn_x <= game_x <= next2_btn_x + next2_btn_w and next2_btn_y <= game_y <= next2_btn_y + next2_btn_h:
                        pygame.mixer.music.stop()
                        return "next_level" if current_level_index < len(LEVEL_ORDER) - 1 else "quit"
                else:
                    if end_btn_x <= game_x <= end_btn_x + end_btn_w and end_btn_y <= game_y <= end_btn_y + end_btn_h and current_level_index > 0:
                        pygame.mixer.music.stop()
                        return "prev_level"
                    if next_btn_x <= game_x <= next_btn_x + next_btn_w and next_btn_y <= game_y <= next_btn_y + next_btn_h and current_level_index < len(LEVEL_ORDER) - 1:
                        pygame.mixer.music.stop()
                        return "next_level"
