...
def run_level(screen, camera_manager, fullscreen_manager, level_name):
    """執行單一關卡的遊戲主迴圈"""
    ...
    while running:
        ...
        if countdown_completed and time.time() - post_countdown_start_time >= 1.5 and not show_finish_popup:
            ...
            # 检查是否可以播放新的声音
            if can_play_sound and not sound_triggered_in_current_window:  # 添加条件：当前窗口未触发过声音
                ...
                if current_action in action_resources:
                    ...
                    if current_action == "open" and open_detected and not prev_open:
                        ...
                        prev_open = True
                    elif (current_action == "head" or current_action.startswith("head")) and head_detected and not prev_head:
                        action_detected = True
                        prev_head = True
                    elif current_action == "left" and left_detected and not prev_left:
                        action_detected = True
                        prev_left = True
                    elif current_action == "right" and right_detected and not prev_right:
                        action_detected = True
                        prev_right = True
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if show_finish_popup:
                    if next2_btn_x <= game_x <= next2_btn_x + next2_btn_w and \
                       next2_btn_y <= game_y <= next2_btn_y + next2_btn_h:
                    ...
                    # 上一關
                    if end_btn_x <= game_x <= end_btn_x + end_btn_w and \
                       end_btn_y <= game_y <= end_btn_y + end_btn_h and \
                       current_level_index > 0:
                        return "prev_level"
                    
                    # 下一關
                    if next_btn_x <= game_x <= next_btn_x + next_btn_w and \
                       next_btn_y <= game_y <= next_btn_y + next_btn_h and \
...
def main():
    ...
    while 0 <= current_level_index < len(LEVEL_ORDER):
        ...
        action = run_level(screen, camera_manager, fullscreen_manager, level_name)
